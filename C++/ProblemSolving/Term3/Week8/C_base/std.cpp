...
void init()
{
	...
	len=strlen(c1);
	dt=log2(len);
	// rlen=(rlen==(1<<dt)?len:1<<(dt+1));  // wrong?
	rlen=(len==(1<<dt)?len:1<<(dt+1));  // rlen is pow of 2
...
int getbase(int n) // tricky
{
	if (n<=16)
	{
		int len=0;
		long long t=1;
		for (int i=1;i<=n;++i) t=t*10;
		do
		{
			base[len++]=t%27;
			t/=27;
...
int trans(int l,int r) 
{
    ...
    int totrf=trans(mid+1,r); // use lf, but not loadlf, because loadlf is temporary
	for (int i=0;i<totlf;++i) lf[i]=loadlf[i]; // recover context
    for(int i=0;i<totrf;i++)
    {
    	rf[i]=s[i];
    }
